                    ...
                    <a href="{{ route('product.show', $product->id) }}">
                        <div class="relative w-full h-full rounded overflow-hidden shadow-lg my-4 border-gray-100 dark:border-gray-700 border">
                            ...
                            <p class="font-bold text-lg p-4 mb-2">
                                <span class="block sm:hidden">{{ Str::limit($product->name, 30, '...') }}</span>
                                <span class="hidden sm:block md:hidden">{{ Str::limit($product->name, 20, '...') }}</span>
                                <span class="hidden md:block">{{ Str::limit($product->name, 25, '...') }}</span>
                            </p>
                            <div class=" right-2 bottom-2 px-4">
                                <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-500">Rp{{ number_format($product->price, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </a>
                    @endforeach
